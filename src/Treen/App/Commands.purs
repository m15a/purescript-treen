...
import Treen.Data.GitLog.Oneline as CLO
...
data Command
  ...
  | OnelineGitLogCommand { input :: Input, tileset :: Tileset }
  ...
  ss <- case input of
    ...
    FilesInput files -> A.foldM step [] $ A.fromFoldable files
      ...
      step a f = do
        ...
        pure $ a <> S.lines s
  let
    treen = T.bundle $ A.mapMaybe (\s -> CLO.fromString s >>= L.fromOnelineGitLog) ss
    out = T.printWith (tilesetOf tileset) S.trim treen
  write stdout =<< fromStringUTF8 (out <> "\n")
